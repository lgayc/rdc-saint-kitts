/* =====================================================================
   RDC_API  ·  VERSION 3  ·  2026-08-17  ·  pares EN/ES
   =====================================================================
   COMO COMPROBAR QUE ESTE ARCHIVO ES EL QUE ESTA EN EL SITIO
   Abre en el navegador:  <tu-dominio>/js/supabase-api.js
   Si la primera linea no dice VERSION 3, lo que hay publicado es una
   copia vieja — o el navegador te la esta dando de su cache.

   Esto esta aqui porque hizo falta: una version 2 del panel con una
   version 1 de este archivo mandaba el objeto { en, es } entero a una
   columna de texto, y la portada acababa enseñando {"en":"..."} a los
   pacientes. Sin una marca visible no habia forma de saber cual de los
   dos archivos estaba publicado.
   ===================================================================== */
...
const RDC_API = (function () {
  ...
  async function getPublicContent() {
    ...
    /* ------------------------------------------------------------
       RED CONTRA UN CASO QUE YA PASO EN PRODUCCION
       ------------------------------------------------------------
       Si el sitio se queda a medio actualizar —el panel nuevo pero
       esta capa vieja— el panel manda { en, es } a una columna de
       texto y Postgres no se queja: guarda el objeto serializado.
       La portada acaba enseñando {"en":"...","es":""} tal cual, en
       el titular, delante de los pacientes. Paso de verdad.

       Asi que al LEER se comprueba: si lo que viene es justo ese
       objeto convertido en texto, se deshace. Cuesta un startsWith
       por campo y evita que una subida incompleta se vea como una
       pagina rota.

       Esto no sustituye a arreglar los datos —eso se hizo aparte,
       con SQL— pero hace que el sitio aguante la proxima vez.
       ------------------------------------------------------------ */
    const desempaquetar = (valor) => {
      if (typeof valor !== "string") return valor;
      const t = valor.trim();
      if (!t.startsWith('{"en"') && !t.startsWith('{"es"')) return valor;
      try {
        const o = JSON.parse(t);
        if (o && typeof o === "object" && ("en" in o || "es" in o)) return o;
      } catch {
        // No era JSON: es texto que casualmente empieza asi. Se deja.
      }
      return valor;
    };

    const par = (en, es) => {
      const a = desempaquetar(en);
      // Si en la columna inglesa venia el par entero, ese par manda:
      // el valor suelto de español es de antes del enredo.
      if (a && typeof a === "object") {
        return { en: a.en || "", es: a.es || es || "" };
      }
      return { en: a || "", es: es || "" };
    };
